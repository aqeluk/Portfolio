...
const ResumeCard = ({ badge, title, subTitle, des }) => {
  const presentBadge = badge.endsWith('Present');

  return (
    <div className='w-full py-6 flex flex-col gap-2.5 border-b-[1px] border-b-zinc-800'>
      <h6
        className={`w-24 text-center text-sm py-[1px] ${
          presentBadge ? 'text-yellow-500' : 'text-[#999]'
        } border-[1px] ${presentBadge ? 'border-yellow-500' : 'border-zinc-600'} rounded-sm`}
      >
        {badge}
      </h6>
